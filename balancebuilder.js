...
function zMegaCheckPassed(mon, stats) {
...
function getRandomMon(team, stats) {
    let a = 0;
    let completed = false;
    while (!completed) {
        let rand = sets[getRandomInt(sets.length - 1)];
        if (isValid(rand.set.name, team) && zMegaCheckPassed(rand, stats) && clericTest(rand, stats)) {
